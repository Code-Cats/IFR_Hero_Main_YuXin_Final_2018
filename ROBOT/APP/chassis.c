	...
	if(GetWorkState()==WAIST_STATE)
	{
		...
		switch(turn_flag)
		{
			case 0:
			{
				...
				if(abs(YAW_INIT-yunMotorData.yaw_fdbP)<58)	//优化-大于一个0附近 负值就可以
				{
					turn_flag=1;
				}
				break;
			...
			{
				YAW_INIT=yaw_init_def+WAIST_RANGE;
				if(abs(YAW_INIT-yunMotorData.yaw_fdbP)<58)	//优化-小于一个0附近的正值
				{
					turn_flag=0;
				}
				break;
...
//	limit_k=0.9;	//取消功率限制，屏蔽后取消
	...
	{
		limit_power_statu=2;
		limit_k=3.0f*powerbuffer/125.0f+0.16f;//+0.25f;	//30j开始限制
		limit_k=limit_k>1?1:limit_k;
		limit_k=limit_k<0.1f?0.1f:limit_k;
	}
